                        ...
                        <div class="d-flex align-items-start mt-3 mt-sm-0">
                            <div class="flex-shrink-0">
                                <div class="avatar-xl me-3">
                                    <img src="{{ asset ('core/images/avatar.png')}}" alt="" class="img-fluid rounded-circle d-block">
                                </div>
                            </div>
                    ...
                    <div class="invoice-title">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-6">
                                                <div>
                                                    ...
                                                    <h5 class="font-size-14 mb-2">Kode Payment</h5>
                                                    <p class="mb-1">{{$dt -> kode_payment}}</p>
                                                    <h5 class="font-size-14 mb-2 mt-3">Kode Transaksi</h5>
                                                    <p class="mb-1">{{$dt -> kode_trx}}</p>
                                                </div>
                                                ...
                                                <table class="table table-nowrap align-middle mb-0">
                                                    ...
                                                    @foreach($dt->details as $data)            
                                                        <tr>
                                                            ...
                                                            <td class="text-end">{{"Rp.".number_format($data->produk->harga * $data->total_item )}}</td>
                                                        </tr>
                                                    @endforeach
                                                        <tr>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="border-0 text-end"><h4 class="m-0">{{"Rp.".number_format($dt->total_harga)}}</h4></td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row" colspan="4" class="border-0 text-end">Bayar</th>
                                                            <td class="border-0 text-end">
                                                                <input type="number" name="payment" id="payment">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row" colspan="4" class="border-0 text-end">Kembalian</th>
                                                            <td class="border:none text-end">
                                                                <input type="number" name="change" id="change" readonly style="background: #e9e9e9">
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                        ...
                                        <div class="d-print-none mt-3">
                                            <div class="float-end">
                                                ...
                                                <a href="javascript: void(0)" class="btn btn-primary w-md waves-effect waves-light" id="btn-checkout">CHECKOUT</a>
                                                @elseif($dt->status == "SELESAI")
                                                <a href="#" class="btn btn-primary w-md waves-effect waves-light disabled">CHECKOUT COMPLETED</a>
...
<script>
    let inputPayment = document.querySelector("#payment");
    let inputChanges = document.querySelector('#change');
    let btnCheckout = document.querySelector('#btn-checkout');

    inputPayment.addEventListener('keyup', () => {
        let total = "{{ $dt->total_harga }}"

        let change = parseInt(inputPayment.value) - parseInt(total);

        inputChanges.value = change;

        disableCheckoutWhenMin(change);

    });

    function disableCheckoutWhenMin(total) {
        if(total < 0){
            btnCheckout.setAttribute('disabled', true)
            btnCheckout.style.background = '#e9e9e9';
            btnCheckout.style.borderColor = '#e9e9e9';
            btnCheckout.style.color = '#000';
        }else{
            btnCheckout.setAttribute('disabled', false)
            btnCheckout.setAttribute('href', "{{ route('transaksiSelesai', Crypt::encryptString($dt->id)) }}")
            btnCheckout.style.background = '#5156be';
            btnCheckout.style.borderColor = '#5156be';
            btnCheckout.style.color = '#fff';
        }
    }

    disableCheckoutWhenMin(-1)
